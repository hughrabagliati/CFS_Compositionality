...
skewness(sense.pop.sklar$rt,na.rm = T)
...
es.t.test.raw <- data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                            r = cor.test(subset(sense.pop.sklar.summary, Condition == "Sklar_control")$rt,subset(sense.pop.sklar.summary, Condition == "Sklar_violation")$rt)$estimate)
...
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                            r = cor.test(subset(sense.pop.new.summary, Condition == "Non-sensible")$rt,subset(sense.pop.new.summary, Condition == "Sensible")$rt)$estimate))
...
sense.sklar.graph$Experiment <- "Study 1 \nIncongruent Sentences"
...
sense.new.graph$Experiment <- "Study 2 \nReversible Sentences"
...
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],
                                                 n2 = summary(es)$df[2],
                                                 r = 0.5))
# note that r = 0.5 here because it is not a within subjects test
...
es <- lm(rt~MeanAffectivity, data = emo.pop.sklar.sum)
es.t.test.log <- rbind(es.t.test.log, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],
                                                 n2 = summary(es)$df[2],
                                                 r = 0.5))
...
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],
                                                 n2 = summary(es)$df[2],
                                                 r = 0.5))
 # Note that r = 0.5 here because this is not a within subjects test
...
emo.pop.hebr.sum$Experiment <- "Study 5 \nHebrew Phrases"

emo.pop.sklar.sum.graph$Experiment <- "Study 3 \nTwo Word Phrases"
emo.pop.sklar.sum.graph$Contrast <- 50
emo.pop.new.sum.graph$Experiment <- "Study 4 \nReversible Sentences"
emo.pop.new.sum.graph$Contrast <- 50

graph <- rbind(emo.pop.sklar.sum.graph,emo.pop.new.sum.graph,emo.pop.hebr.sum)

graph <- na.omit(graph)
graph$Experiment <- ordered(graph$Experiment, levels = c("Study 3 \nTwo Word Phrases", "Study 4 \nReversible Sentences", "Study 5 \nHebrew Phrases"))
...
es.t.test.raw <- rbind(es.t.test.raw,data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                                                r = cor.test(subset(sklar.sem.sum, Condition == "Control")$rt,subset(sklar.sem.sum, Condition == "Violation")$rt)$estimate))
...
es.t.test.raw <- rbind(es.t.test.raw,data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                                                r = cor.test(subset(edin.sem.sum, Condition == "Control")$rt,subset(edin.sem.sum, Condition == "Violation")$rt)$estimate))
...
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],
                                                 n2 = summary(es)$df[2], r = 0.5))
# r = 0.5 because this is not a within subjects effect
...
es <- lm(rt~MeanAffectivity_column, data = sklar.emo.sum)
es.t.test.log <- rbind(es.t.test.log, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],n2 = summary(es)$df[2], r = 0.5))
...
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = summary(es)$coefficients[2,3],n1 = summary(es)$df[2],
                                                 n2 = summary(es)$df[2], r = 0.5))
# r = 0.5 because this is not a within subject effect
...
sklar.emo.sum.graph$Experiment <- "Study 8\n(Study 3 Replication)\nTwo Word Phrases"
edin.emo.sum.graph$Experiment <- "Study 9\n(Study 4 Replication)\nReversible Sentences"

graph <- rbind(sklar.emo.sum.graph,edin.emo.sum.graph)

graph <- na.omit(graph)
graph$Experiment <- ordered(graph$Experiment, levels = c("Study 8\n(Study 3 Replication)\nTwo Word Phrases", "Study 9\n(Study 4 Replication)\nReversible Sentences"))
...
sense.sklar.graph$Experiment <- "Study 6\n(Study 1 Replication)\nIncongruent Sentences"
...
sense.new.graph$Experiment <- "Study 7\n(Study 2 Replication)\nReversible Sentences"
...
# Run t tests for meta-analysis

es <- t.test(rt ~ prime_semantics, data = subset(words.sum, length == "Long"), paired = T)
# multiply es$t by -1, because in this experiment 
# the factors levels are reversed from prior studies (because we use negative/neutral rather than 
# negative controls)
es$statistic <- es$statistic * -1
es.t.test.raw <- rbind(es.t.test.raw,data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                                                r = cor.test(subset(words.sum, length == "Long" & prime_semantics == "negative")$rt,subset(words.sum, length == "Long" & prime_semantics == "neutral")$rt)$estimate))

es <- t.test(rt ~ prime_semantics, data = subset(words.sum, length == "Short"), paired = T)

es$statistic <- es$statistic * -1
es.t.test.raw <- rbind(es.t.test.raw,data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
                                                r = cor.test(subset(words.sum, length == "Short" & prime_semantics == "negative")$rt,subset(words.sum, length == "Short" & prime_semantics == "neutral")$rt)$estimate))
...
es <- t.test(rt ~ prime_semantics, data = subset(words.sum, length == "Long"), paired = T)
# multiply es$t-statistic by -1, because in this experiment 
# the factors levels are reversed from prior studies (because we use negative/neutral rather than 
# negative controls)
es$statistic <- es$statistic * -1
es.t.test.log <- rbind(es.t.test.log,data.frame(t = es$statistic,n1 = es$parameter,n2 = es$parameter,
...
sense.graph$rt <- sense.graph$rt * 1000

sense.graph <- summaryBy(rt ~ prime_semantics +length, data = sense.graph, FUN = c(mean,ci.high,ci.low))

sense.graph$length <- ordered(sense.graph$length, 
                              levels = c("Short","Long"), 
                              labels = c("Short words", "Long words"))
...
# Add t.tests from Sklar Experiments (n =4), yang yeh experiments (n=2)
# Because I am uncertain about d from partial eta squared (there's some debate about this)
# we first transform F to t (t = sqrt(F))
# and then calculate as with all the others, assuming a correlation of about 0.95
# For Expt 1, F = 20.09, t = -4.48 (make -ve to account for directionality)
# For Expt 2, F = 18.35, t = -4.28
es.t.test.raw <- rbind(es.t.test.raw, data.frame(t = c(2.63,2.92,2.523,2.229,-4.48,-4.28), n1 = c(31,21,45,45,11,11), n2 = c(31,21,45,45,11,11), r = c(0.975,0.975,0.5,0.5,0.975,0.975)))
es.t.test.raw
raw.es <- tes(es.t.test.raw[,1],es.t.test.raw[,2],es.t.test.raw[,3], verbose = FALSE)
raw.es$exp <- c("Study 1", "Study 2","Study 3", "Study 4","Study 6","Study 7","Study 8","Study 9","Study 10a","Study 10b", "Sklar et al. Expt. 1","Sklar et al. Expt. 2", "Sklar et al. Expt. 4a","Sklar et al. Expt. 4b", "Yang & Yeh Expt. 1","Yang & Yeh Expt. 2")
...
raw.es$r <- es.t.test.raw$r
raw.es$t <- es.t.test.raw$t

# Recalculate d using correlations between conditions for within subjects comparisons
#  (i.e., Experiments 1a and 1b, Experiments 3a and 3b, and the two conditions of Experiment 4, plus
# Estimate the correlations for the Sklar and Yang & Yeh data)

raw.es$d <- raw.es$t * sqrt(2*(1-raw.es$r)/raw.es$n.1)
raw.es$d.se <- sqrt((2 * (1 - raw.es$r)/raw.es$n.1) + 
                      (raw.es$d^2)/(2 * raw.es$n.1))

#raw.es
# we originally ran a random effects meta analysis on unadjusted t's, 
# but then realised that over-estimated effect size! Now, we adjust for
# correlations so need to use a different metaregression structure
#ran.ef.raw <- rma(d,var.d,random = ~ 1|lab,data = raw.es)
ran.ef.raw <- rma.mv(d,V= (d.se)^2, random = ~ 1|lab,data = raw.es)
ran.ef.raw
forest(ran.ef.raw,
       slab = raw.es$exp,
       mlab = "All Studies",
...
par(font = 2)
text(-1.35, 17.55, "All studies (raw data)")
text(1.6, 17.55, "Cohen's d [95% CI]")
par(font = 1)
funnel(ran.ef.raw)

```

We then calculated a Bayes factor for this regression, comparing a null hypothesis to a hypothesis within which the true effect of unconscious semantics was uniformly distributed somewhere between -1 and 1, i.e., a very non-informative theory which could account for the wide variety in effect sizes found in this literature. The calculator at 
[Zoltan Dienes' site](http://www.lifesci.sussex.ac.uk/home/Zoltan_Dienes/inference/Bayes.htm) produced a Bayes factor of 0.24, suggesting that the data is approximately four times more likely under the null hypothesis than the non-informative theory.

We also used Richard Morey's method for calculating meta-analytic BayesFactors. This function takes t scores and computes effect sizes from them, to produce a meta-analytic Bayes factor. But it assumes that the t scores are from between-subjects t tests, so we first use our effect size estimates to generate the t scores that would have been expected from our effect sizes if our tests had been between-subjects.

```{r bayes_factor_morey_method}

raw.es$t_adjusted = raw.es$d * sqrt(raw.es$n.1) 
#raw.es
summary(meta.ttestBF(t = raw.es$t_adjusted,n1=raw.es$n.1,n2=raw.es$n.2,nullInterval = c(-1,1)))
summary(meta.ttestBF(t = raw.es$t_adjusted,n1=raw.es$n.1,n2=raw.es$n.2))
...
#No longer need to add Expt 4 manually
...
log.es$exp <- c("Study 1", "Study 2","Study 3", "Study 4","Study 6","Study 7","Study 8","Study 9","Study 10a","Study 10b")
log.es$lab <- c(rep("Ed",10))#,rep("Israel",4))
log.es$procedure <- c("1","2","3","4","1","3","2","4","5","5")
#kable(log.es)
log.es$r <- es.t.test.log$r
log.es$t <- es.t.test.log$t

log.es$d <- log.es$t * sqrt(2*(1-log.es$r)/log.es$n.1)
log.es$d.se <- sqrt((2 * (1 - log.es$r)/log.es$n.1) + 
                      (log.es$d^2)/(2 * log.es$n.1))
#log.es
# Old style analysis before accounting for correlations
#ran.ef.log <- rma(d,var.d,random = ~ 1|procedure, data = log.es)
ran.ef.log <- rma.mv(d,V= (d.se)^2, random = ~ 1|procedure,data = log.es)
ran.ef.log
forest(ran.ef.log,
       slab = log.es$exp,
       mlab = "All Studies",
       xlab = "Cohen's d", psize = 1.5)
par(font = 2)
text(-.77, 11.6, "Edinburgh Studies (log transformed data)")
text(1.1, 11.6, "Cohen's d [95% CI]")
funnel(ran.ef.log)
```


We then calculated a Bayes factor for this regression, comparing a null hypothesis to a hypothesis within which the true effect of unconscious semantics was uniformly distributed somewhere between -1 and 1, i.e., a very non-informative theory which could account for the wide variety in effect sizes found in this literature. The calculator at 
[Zoltan Dienes' site](http://www.lifesci.sussex.ac.uk/home/Zoltan_Dienes/inference/Bayes.htm) produced a Bayes factor of 0.02, suggesting that the data is approximately 50 times more likely under the null hypothesis than the non-informative theory.

We also used Richard Morey's method for calculating meta-analytic BayesFactors

```{r bayes_factor_morey_method_low}
log.es$t_adjusted = log.es$d * sqrt(log.es$n.1) 
#log.es

summary(meta.ttestBF(t = log.es$t_adjusted,n1=log.es$n.1,n2=log.es$n.2,nullInterval = c(-1,1)))
```
