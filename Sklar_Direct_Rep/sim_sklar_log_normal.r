...
sigma <- 0.34 
tau <- 2.6765
subj_range<- data.frame(subj = seq(10,100,by = 5),rt.p.noexcl = NA, log_rt.p.noexcl = NA, rt.p.excl1 = NA,rt.p.excl2 = NA,rt.p.excl3 = NA,rt.p.excl4 = NA,rt.p.full = NA)
...
for (l in subj_range$subj){
  ...
  for (i in 1:no.sim){
    ...
    for (k in 1:no.subj){
      ...
      for (j in 1:no.conditions){
        data[data$subj == k & data$cond == j,]$rt <- rlnorm(no.trials, mu.subj,sigma.subj)
      }
    }
