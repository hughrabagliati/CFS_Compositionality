...
sigma <- 0.0974 
tau <- 2.6765
subj_range<- data.frame(subj = seq(30,60,by = 30),rt.p.noexcl = NA, log_rt.p.noexcl = NA, rt.p.excl1 = NA,rt.p.excl2 = NA,rt.p.excl3 = NA,rt.p.excl4 = NA,rt.p.full = NA)
...
for (i in 1:no.sim){
	...
	for (k in 1:no.subj){
		...
		for (j in 1:no.conditions){
			...
			data[data$subj == k & data$cond == j,]$rt <- rexgauss(no.trials, (mu.subj+0.1),sigma.subj,(tau.subj+0.1))
			}
		}
	}
