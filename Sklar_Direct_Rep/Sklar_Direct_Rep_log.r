...
summary(lmer(rt ~ MeanAffectivity_column +Length +(1+MeanAffectivity_column|SubjNo) + (1|Stim_column), data = sklar.emo))
#############
# Edinburgh Emo Expt
...
# Rsemve RTs < 200ms & > 10
edin.sem <- subset(edin.sem, rt > log(0.2))
edin.sem <- subset(edin.sem, rt <= log(10))
